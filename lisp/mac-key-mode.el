...
(defvar mac-key-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map [(alt b)] 'dired-up-directory)
    ...
    (define-key map [(alt t)] 'git-gutter)
    ...
    ;; (define-key map [(alt up)] 'git-gutter:previous-hunk)
    ;; (define-key map [(alt down)] 'git-gutter:next-hunk)
    (define-key map [(alt up)] 'diff-hl-previous-hunk)
    (define-key map [(alt down)] 'diff-hl-next-hunk)
